...
- 🃏 **原生卡牌图鉴**：新增《杀戮尖塔 2》图鉴页，支持画廊浏览、详情查看、升级态切换与本地化文本展示。
- 🎨 **游戏内卡面资源提取**：直接从本地游戏提取卡图、边框、横幅、能量图标与标题字体，卡面表现更贴近游戏内原始资源。
- 🔄 **本地化刷新链路**：刷新资源时会从本地游戏重新生成图鉴快照并缓存，不再依赖仓库内置的静态 `card-metadata` 文件。
- 🧭 **图鉴浏览体验优化**：新增吸顶筛选栏、折叠筛选区和回到顶部按钮，长列表浏览更顺手。
- 💾 **Profiles 自动保存**：编辑现有配置档时，模组选中变更会自动持久化，减少反复手动保存。
